...
p.setAdditionalSearchPath(
    r"C:\Users\vssri\Desktop\Task_2\task 2.2\AMC_Winter-Camp-2025-26\Task\Week-2\Task-2.2\src"
)
...
car = p.loadURDF("car\car1.urdf", carPos, p.getQuaternionFromEuler([0,0,angle]))  #Loading the car with head parallel to the given line
...
def moveCar(base_torque, action):  #Enter the motor control here to move the car, give base torque and action calculated as input
    ...
    p.setJointMotorControlArray(car, [fl, bl, fr, br], p.TORQUE_CONTROL, forces = [base_torque +action,base_torque + action ,base_torque-action,base_torque-action ])
...
def calc_error(): #You can calculate the error and required action using this function
    ...
    position , orientn = p.getBasePositionAndOrientation(car)
    error = position[1] - desired_state
...
while(True):                         #This while loop will run until ESCAPE key is pressed, then it will start the simulation.
    ...
    if keycode.get(p.B3G_RETURN) == 1:                     #As soon as any key is pressed and it's ENTER key, simulation starts
        ...
        car = p.loadURDF("car\car1.urdf", carPos, p.getQuaternionFromEuler([0,0,angle]))   #Plane and car loaded again
        p.setJointMotorControlArray(car, [fl, bl, fr, br], p.VELOCITY_CONTROL, forces = [0,0,0,0])   #This is done to enable torque control in wheels of the car
        ...
        while(True):
            ...
            error= calc_error()
            action = ((P*error) + I*total_error + D*(error - prev_error))
            moveCar(base_torque,action)

            prev_error = error
            total_error += error
           #Result: Took 1 minute to align itself on the line in the best attempt, then constantly stayed on it
           #D is scaled keeping in mind that division by dt is included in it
